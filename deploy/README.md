...
| `api`      | `quantbot-app` | FastAPI control plane                     | 127.0.0.1:8000   |
...
> **Start** in the dashboard — even with `BOT_MODE=LIVE`. LIVE just wires the real
...
$EDITOR .env                        # set BYBIT_API_KEY/SECRET, BOT_MODE=LIVE
...
starts the stack, installs/reloads the host nginx vhost, and waits for `/health`.
...
# start LIVE trading (or use the dashboard Start button)
curl -X POST http://127.0.0.1:8000/bot/start \
     -H 'Content-Type: application/json' \
     -d '{"mode":"LIVE","live_confirm":true}'
