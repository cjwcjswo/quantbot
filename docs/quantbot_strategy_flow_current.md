...
| compression_min_score | 6 |
| no_compression_min_score | 7 |
| compression_position_fraction | 0.30 |
| no_compression_position_fraction | 0.20 |
| LONG RSI 범위 | 46-64 |
| SHORT RSI 범위 | 36-54 |
| 주문 방식 | LIMIT |
| TTL | 30초 |
...
| 박스 매우 근접 | 2점 |
| 박스 중간 근접 | 1점 |
| ATR20 < ATR100 compression 존재 | 2점 |
| volume_ratio 높음 | 2점 |
| volume_ratio 기준 통과 | 1점 |

compression이 있으면 `compression_min_score=6`, `compression_position_fraction=0.30`을 사용한다. compression이 없어도 Scout 평가는 계속 진행하지만 `no_compression_min_score=7`을 요구하고, 통과하더라도 `no_compression_position_fraction=0.20`으로 더 작게 진입한다. compression이 없고 점수가 부족하면 `SCOUT_SCORE_TOO_LOW_NO_COMPRESSION`으로 기록한다.
