...
1. 거래소 포지션을 재조회하여 실제 size와 average price를 확인한다.
2. Bybit `set_trading_stop`으로 SL을 설정한다.
3. SL 상태를 다시 조회하여 검증한다.
4. 검증 성공 시 position status를 `ACTIVE`로 변경한다.
5. 검증 실패 시 emergency close를 수행한다.
