...
현재 healthy breakout 판정은 `entry.breakout_confirm.volume_min_ratio = 1.3`을 사용한다.
...
| max_distance_to_box_atr | 0.65 ATR |
| require_compression | false |
| compression_bonus_score | 2 |
| compression_min_score | 5 |
| no_compression_min_score | 7 |
| compression_position_fraction | 0.50 |
| no_compression_position_fraction | 0.15 |
| LONG RSI 범위 | 44-68 |
| SHORT RSI 범위 | 32-58 |
| 주문 방식 | LIMIT |
| TTL | 45초 |
...
- volume_ratio가 0.8 이상 4.0 미만
- compression은 필수 차단 조건이 아니라 점수와 수량을 조절하는 조건
- Anti-chase 통과 필요

LONG Scout 조건:

- close가 box_high 이하
- box_high - close <= 0.65 ATR
...
| 박스 매우 근접 | 2점 |
| 박스 중간 근접 | 1점 |
| ATR20 / ATR100 <= 0.8 compression 존재 | 2점 |
| volume_ratio 높음 | 2점 |
| volume_ratio 기준 통과 | 1점 |

compression이 있으면 `compression_min_score=5`, `compression_position_fraction=0.50`을 사용한다. compression이 없어도 Scout 평가는 계속 진행하지만 `no_compression_min_score=7`을 요구하고, 통과하더라도 `no_compression_position_fraction=0.15`로 더 작게 진입한다. compression이 없고 점수가 부족하면 `SCOUT_SCORE_TOO_LOW_NO_COMPRESSION`으로 기록한다.
...
| --- | --- | --- | --- | --- |
| Scout | LIMIT | BUY=best_bid, SELL=best_ask | GTC, 45초 TTL | 미체결 시 취소 후 1회 재시도 |
...
| `SCOUT_SCORE_TOO_LOW_NO_COMPRESSION` | compression 없는 Scout가 더 높은 필요 점수를 채우지 못함 |
...
| SCOUT_ZONE | boundary까지 `entry.pre_breakout.max_distance_to_box_atr`, 현재 0.65 ATR 이하 |
| WATCHING | 아직 멀리 있음 |
| NO_SIGNAL | 전략 신호 없음 |
